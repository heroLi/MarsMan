...
public class User {
    ...
    public String toString() {
    	...
    	StringBuffer buffer = new StringBuffer("User String = ");
    	buffer.append(" 用户UID（int64）=").append(id);
    	buffer.append(" 字符串型的用户 UID=").append(idstr);
    	buffer.append(" 用户昵称=").append(screen_name);
    	buffer.append(" 友好显示名称=").append(name);
    	buffer.append(" 用户所在省级ID=").append(province);
    	buffer.append(" 用户所在城市ID=").append(city);
    	buffer.append(" 用户所在地=").append(location);
    	buffer.append(" 用户博客地址=").append(url);
    	buffer.append(" 用户头像地址，50×50像素=").append(profile_image_url);
    	buffer.append(" 用户的微博统一URL地址=").append(profile_url);
    	buffer.append(" 用户的个性化域名=").append(domain);
    	buffer.append(" 用户的微号=").append(weihao);
    	buffer.append(" 性别，m：男、f：女、n：未知=").append(gender);
    	buffer.append(" 粉丝数 =").append(followers_count);
    	buffer.append(" 关注数=").append(friends_count);
